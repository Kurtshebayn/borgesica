...
Change: `translation-engine` · Capability: `context-continuity`
...
These rules SHALL appear verbatim in the static instruction block of the system prompt. The eval harness (M4) SHALL score `neutral-register` as a named dimension, with a score ≥ 4/5 on the LLM-as-judge rubric considered passing.
...
#### Scenario: eval harness scores neutral-register dimension (M4)
